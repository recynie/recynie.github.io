---
permalink: /
title: "About Me"
author_profile: true
redirect_from: 
  - /about/
  - /about.html
---

I am an undergraduate student majoring in Artificial Intelligence at [School of Future Technology](https://futureschool.dlut.edu.cn/), [Dalian University of Technology](https://www.dlut.edu.cn/).
My research interests center on **autonomous scientific discovery** and **recursive self-mprovement**.

The [publications page]({{ '/publications/' | relative_url }}) shows details of works I evolved, and the [projects page]({{ '/portfolio/' | relative_url }}) shows my hobby projects and ongoing researches.
I also write on my [personal blog](https://recynie.github.io/blog/).

Research Interests
======

- AI for Scientific Research: AI scientist, autonomous scientific research agent
- Recursive Self-Improvement (RSI): self-improvement agent, external knowledge evolution
- AI for Science: protein hydration embedding, protein optimization, molecular representation

News
======

- **May. 2026** Our paper [KnowEvo: Knowledge Evolution for Protein Optimization]({{ '/publication/KnowEvo' | relative_url }}) submitted to *NeurIPS 2026 Conference*.
- **Mar. 2026** Our paper [Language-guided expertise evolution for protein optimization]({{ '/publication/language-guided-protein-optimization' | relative_url }}) was accepted as a poster at the *ICLR 2026 Workshop on AI with Recursive Self-Improvement*.
- **Jan. 2026** I was invited by MiraclePlus (奇绩创坛) for an interview on protein optimization and ranked in the top 10%.
- **Jun. 2025** I received First Prize in the Dalian University of Technology Information Security Competition.
- **Jun. 2025** Our paper [Motif-driven molecular graph representation learning]({{ '/publications/motif-driven-molecular-graph-representation-learning' | relative_url }}) was accepted by *Expert Systems With Applications*.

Current Work
======

- *(Ongoing)* AI for scientific research: agents that propose diverse and novel ideas, and recursively refine them with feedback.
- *(Ongoing)* Hydration embedding for enzyme optimization: Hydration representations can improve the performance of enzyme activity prediction (\\(k_m\\), \\(k_{cat}\\)) tasks.
- Knowledge evolution for protein optimization: external knowlege representation that evolves from computational feedback for protein optimization tasks, featuring explainable knowledge and replaceble computational backends.
- Molecular representation learning for LLMs and GNNs.

For paper-level details, please see the [publications page]({{ '/publications/' | relative_url }}) or my [Google Scholar profile](https://scholar.google.com/citations?user=-9kSV0IAAAAJ&hl).

Selected Honors
======

- National Scholarship, 2024-2025
- Academic Excellence Scholarship, 2024-2025
- Academic Excellence Scholarship, 2023-2024
